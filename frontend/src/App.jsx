...
// ─── CONV LIST ───────────────────────────────────────────────────────
function ConvList({ convs, selected, setSelected, filter, setFilter, onNewChat }) {
  ...
  function handleCreate() {
    ...
    onNewChat({ ...form });
    setForm({ name: "", phone: "", channel: "WhatsApp" });
    setShowModal(false);
  }
    ...
    <div style={{ width: 272, borderRight: `1px solid ${C.gray200}`, display: "flex", flexDirection: "column", background: C.white, flexShrink: 0 }}>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: C.white, borderRadius: 16, padding: 24, width: 320, boxShadow: "0 8px 30px rgba(0,0,0,0.15)" }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: C.gray800, marginBottom: 16 }}>💬 Chat Baru</div>
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.gray600, marginBottom: 4 }}>Nama Pelanggan</div>
            ...
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.gray600, marginBottom: 4 }}>Nomor HP</div>
              <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                placeholder="contoh: 6281234567890"
                style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.gray600, marginBottom: 4 }}>Channel</div>
              <select value={form.channel} onChange={e => setForm(f => ({ ...f, channel: e.target.value }))}
                style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, fontSize: 13, outline: "none", background: C.white }}>
                <option>WhatsApp</option>
                <option>Website</option>
              </select>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, background: C.white, cursor: "pointer", fontWeight: 600, fontSize: 13, color: C.gray600 }}>Batal</button>
              <button onClick={handleCreate} style={{ flex: 1, padding: "9px", borderRadius: 10, border: "none", background: C.primary, color: "#fff", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>Buat Chat</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: "14px 12px 10px", borderBottom: `1px solid ${C.gray200}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: C.gray800 }}>Percakapan</div>
          <button onClick={() => setShowModal(true)} style={{ background: C.primary, color: "#fff", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>+ Baru</button>
        </div>
...
export default function App() {
  ...
  function addNewConv({ name, phone, channel }) {
    const newId = Date.now();
    const newConv = {
      id: newId, name, phone, channel,
      status: "bot", assignedTo: null, unread: 0,
      lastMsg: "Percakapan baru", lastTime: new Date().toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }),
      tag: "Baru",
      messages: [],
      profile: { totalOrder: 0, totalSpend: 0, joinDate: new Date().toLocaleDateString("id-ID", { month: "long", year: "numeric" }), notes: "-" }
    };
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Inter','Segoe UI',sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          {tab === "inbox" && (
            <>
              <ConvList convs={convs} selected={selectedId} setSelected={setSelectedId} filter={filter} setFilter={setFilter} onNewChat={addNewConv} />
